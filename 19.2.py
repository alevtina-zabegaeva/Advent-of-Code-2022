...
def main():
    ...
    with open(filename) as f:
        for line in f:
            ...
            blueprients.append(((numbers[0], 0, 0, 0),
                                (numbers[1], 0, 0, 0),
                                (numbers[2], numbers[3], 0, 0),
                                (numbers[4], 0, numbers[5], 0)))

    max_geodes = []
    for costs in blueprients[:min(3, len(blueprients))]:
            ...
            for way in tree:
                for r in range(4):
                    if r in (0, 1, 2) and way[3][r] == max_robots[r]:
                        continue
                    if any([costs[r][i] > 0 and way[3][i] == 0 for i in range(3)]):
                        continue
                    delta_t = 1 + max([math.ceil((costs[r][i] - way[2][i]) / way[3][i]) if costs[r][i] > way[2][i] else 0 for i in range(3)])
